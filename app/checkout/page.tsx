...
function StepHeader({ num, title }: { num: string; title: string }) {
  return (
    <div className="flex items-center gap-4 mb-6">
      <span
        className="w-6 h-6 rounded-full border border-[rgba(196,163,115,0.35)] flex items-center justify-center font-display text-brass flex-shrink-0"
        style={{ fontSize: "0.46rem", letterSpacing: "0.05em" }}
      >
        {num}
      </span>
      <h2 className="font-display text-ivory" style={{ fontSize: "0.78rem", letterSpacing: "0.14em" }}>
        {title}
      </h2>
    </div>
  );
...
function Field({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label className="font-display text-[0.52rem] tracking-[0.18em] uppercase text-[rgba(196,163,115,0.50)]">
        {label}
      </label>
      {children}
      {error && (
        <p className="font-body font-light text-[rgba(205,75,75,0.68)] text-[0.76rem]">{error}</p>
      )}
    </div>
  );
}
...
export default function CheckoutPage() {
  ...
  // Prevents the empty-cart guard from firing right after clearCart() on success
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (Object.keys(errs).length > 0) {
      setErrors(errs);
      const firstErr = document.querySelector<HTMLElement>("[data-err]");
    ...
    if (form.paymentMethod === "cod") {
      const { error } = await supabase
        .from("orders")
        .insert({ ...base, payment_status: "pending" });

      if (error) {
        console.error("COD insert error:", error);
        setPaymentError("Unable to place your order. Please try again.");
        setSubmitting(false);
        return;
      }
    ...
    try {
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response: RzpSuccess) => {
          ...
          if (error) {
            setPaymentError(
              `Payment received (ID: ${response.razorpay_payment_id}) but the order record could not be saved. Please contact us with this reference.`
            );
            setSubmitting(false);
            return;
          }
        ...
        modal: {
          ondismiss: () => {
            setPaymentError(
              "Payment was not completed. Your cart is saved — you can try again when ready."
            );
            setSubmitting(false);
          },
        },
  ...
  if (!hydrated || (items.length === 0 && !orderPlacedRef.current)) {
    return (
      <div className="min-h-screen bg-black-plum flex items-center justify-center">
        <div
          className="w-6 h-6 rounded-full border-2 border-[rgba(196,163,115,0.18)] border-t-brass animate-spin"
        />
      </div>
    );
  }
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <Field label="First Name" error={errors.firstName}>
                    <input
                      ...
                      data-err={errors.firstName ? "1" : undefined}
                    />
                  </Field>
                ...
                <div className="flex flex-col gap-5">
                  <Field label="Street Address" error={errors.street}>
                    <input
                      ...
                      placeholder="Flat / house / block, street, area"
                    ...
                    <Field label="State" error={errors.state}>
                      <select
                        ...
                        className={[
                          ...
                          !form.state ? "text-[rgba(245,237,224,0.20)]" : "",
                        ].join(" ")}
                      >
                        <option value="" disabled>Select state</option>
                        {INDIAN_STATES.map((s) => (
                          <option key={s} value={s} className="bg-damson text-ivory">
                            {s}
                          </option>
                        ))}
                      </select>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {(["cod", "online"] as const).map((method) => (
                    <button
                      ...
                      className={[
                        "flex items-start gap-4 w-full text-left px-5 py-4 rounded-[4px] border transition-all duration-200",
                        form.paymentMethod === method
                          ? "border-[rgba(196,163,115,0.55)] bg-[rgba(196,163,115,0.05)]"
                          : "border-[rgba(196,163,115,0.16)] hover:border-[rgba(196,163,115,0.32)]",
                      ].join(" ")}
                    >
                      {/* Radio dot */}
                      <div
                        className="w-4 h-4 mt-0.5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors duration-200"
                        style={{
                          borderColor:
                            form.paymentMethod === method ? "#C4A373" : "rgba(196,163,115,0.28)",
                        }}
                      >
                        {form.paymentMethod === method && (
                      ...
                      <div>
                        ...
                        <p className="font-body font-light italic text-[rgba(245,237,224,0.40)] text-[0.82rem] mt-0.5">
                          {method === "cod"
              ...
              {paymentError && (
                <div className="bg-[rgba(196,163,115,0.05)] border border-[rgba(196,163,115,0.16)] rounded-[4px] px-5 py-4">
                  <p className="font-body font-light text-[rgba(245,237,224,0.62)] text-[0.88rem] leading-[1.75]">
                    {paymentError}
                  </p>
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={submitting}
                className="lg:hidden w-full flex items-center justify-center gap-2.5 font-display text-[0.65rem] tracking-[0.22em] uppercase text-brass border border-[rgba(196,163,115,0.40)] hover:bg-[rgba(196,163,115,0.07)] hover:border-[rgba(196,163,115,0.60)] rounded-[3px] py-4 transition-all duration-200 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {submitLabel}
              </button>
            ...
            <div className="w-full lg:w-[340px] xl:w-[370px] flex-shrink-0 lg:sticky lg:top-24">
              ...
              <div className="bg-damson border border-[rgba(196,163,115,0.14)] rounded-[6px] overflow-hidden">
                ...
                <div className="px-5 pt-5 pb-4 border-b border-[rgba(196,163,115,0.10)]">
                  ...
                  <p className="font-display text-[0.48rem] tracking-[0.14em] uppercase text-[rgba(196,163,115,0.38)] mt-1">
                  ...
                  {items.map((item) => (
                    <div key={item.id} className="flex gap-3.5 px-5 py-3.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className="font-display text-ivory truncate leading-tight"
                          style={{ fontSize: "0.78rem", letterSpacing: "0.04em" }}
                        >
                          {item.name}
                        </p>
                        {item.label && (
                          <p className="font-display text-[0.44rem] tracking-[0.12em] uppercase text-[rgba(196,163,115,0.38)] mt-0.5 truncate">
                            {item.label}
                          </p>
                        )}
                ...
                <div className="px-5 pb-5">
                  <button
                    type="submit"
                    disabled={submitting}
                    className="hidden lg:flex w-full items-center justify-center gap-2.5 font-display text-[0.62rem] tracking-[0.22em] uppercase text-brass border border-[rgba(196,163,115,0.40)] hover:bg-[rgba(196,163,115,0.07)] hover:border-[rgba(196,163,115,0.60)] rounded-[3px] py-3.5 transition-all duration-200 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {submitLabel}
                  </button>
                </div>
              ...
              <Link
                href="/cart"
                className="mt-4 flex items-center gap-2 font-display text-[0.52rem] tracking-[0.16em] uppercase text-[rgba(196,163,115,0.35)] hover:text-[rgba(196,163,115,0.62)] transition-colors duration-200"
              >
                <svg
                  width="10"
                  height="8"
