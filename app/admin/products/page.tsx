...
const EMPTY_DRAWER: DrawerForm = {
  name: "", category: "candle", type: "scented", price: "", mrp: "", stock: "0",
  is_visible: true, is_featured: false, bullet_points: "", sku: "",
  short_description: "", long_description: "", meta_title: "", meta_description: "",
  weight_grams: "", length_cm: "", width_cm: "", height_cm: "", hsn_code: "",
};
...
export default function AdminProductsPage() {
  ...
  useEffect(() => {
    supabase
      .from("products")
      .select("id,name,category,type,stock,price,mrp,image_url,bullet_points,is_visible,is_featured,sku,short_description,long_description,meta_title,meta_description,weight_grams,length_cm,width_cm,height_cm,hsn_code")
  ...
  async function saveBullets(product: Product) {
    ...
    if (error) showToast("Error saving description: " + error.message);
    else {
  ...
  async function saveDrawer() {
    ...
    if (!drawerForm.price || Number(drawerForm.price) <= 0) { showToast("Valid selling price is required."); return; }
  ...
  async function deleteImage(img: DbProductImage) {
    ...
    if (remaining.length === 0) {
      setProducts(prev => prev.map(p => p.id === editingProd.id ? { ...p, image_url: null } : p));
    }
  }
        ...
        <div className="fixed top-0 right-0 bottom-0 w-full max-w-[520px] border-l border-[rgba(196,163,115,0.14)] z-50 flex flex-col overflow-hidden"
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-[rgba(196,163,115,0.10)] flex-shrink-0">
            <div>
              ...
              <p className="font-display text-ivory text-[0.85rem] tracking-wide truncate max-w-[320px]">
            ...
            <section>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input className={FIELD} value={drawerForm.name} onChange={e => setDrawerForm(f => ({ ...f, name: e.target.value }))} placeholder="Nakshatra Candle — Rose" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className={LABEL}>SKU</label>
                    <input className={FIELD} value={drawerForm.sku} onChange={e => setDrawerForm(f => ({ ...f, sku: e.target.value }))} placeholder="NK-ROSE-001" />
                  </div>
                  <div>
