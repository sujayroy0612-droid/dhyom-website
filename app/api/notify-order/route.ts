  ...
  <tr>
    <td style="padding:24px 32px 0;">
      ...
      <p style="margin:0 0 24px;color:rgba(245,237,224,0.60);font-size:14px;line-height:1.7;">
        ...
        ${invoiceNumber ? `<br><br><a href="https://www.dhyom.in/invoice/${data.orderNumber}" style="display:inline-block;background:#c4a373;color:#1a0a12;font-size:13px;font-weight:bold;padding:10px 20px;border-radius:4px;text-decoration:none;letter-spacing:0.5px;">View &amp; Print GST Invoice →</a>` : ""}
      </p>
    </td>
  </tr>
  <tr>
...
async function sendCustomerConfirmation(data: {
  ...
  try {
    ...
    if (custErr) {
      ...
      console.log(`[notify-order/customer-email] Sent → ${data.email} | invoice: ${invoiceNumber ?? "none"}`);
    }
  } catch (err) {
